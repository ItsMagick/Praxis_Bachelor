...
in Kombination des bereits integrierten Monitoring des \gls{zup} etwas langsame ist.
...
In einem Zeitintervall von 48 Stunden wurden bei \gls{afl}Net 151 Bugs gefunden, welche wie bereits in Sektion~\ref{subsubsec:bugs}
genauer beschrieben in drei Bug-Kategorien eingeordnet werden konnten.
Trotz einer niedrigen Anzahl an Ausführungen pro Sekunde konnte \gls{afl}Net bereits in sieben Minuten (vgl.\ Fig.~\ref{fig:aflnet_bugs})
...
strenger definiert werden müssen und nicht nur auf Brute-Force basieren sollten.
