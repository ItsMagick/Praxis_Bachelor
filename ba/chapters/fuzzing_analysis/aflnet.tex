%! Author = chaorn
%! Date = 13.08.24

\subsection{AFLNet}\label{subsec:aflnet}
\gls{afl} ist ein Fuzzer, der auf dem Prinzip der Codeabdeckung basiert.
Er zählt somit zu der Familie der Grey-Box Fuzzer und basiert auf einer mutationsbasierten Fuzzing-Technik.
Bei \gls{afl} handelt es sich ebenso um einen Applikationsfuzzer, welcher mithilfe eines Eingabestreams Daten
an ein \gls{zup} sendet und somit die Software auf Schwachstellen testet.\newline
Zur Analyse des Programms wird in dieser Arbeit jedoch ein Derivat von \gls{afl}(\gls{afl}Net) verwendet.
Es unterscheidet sich von dem ursprünglichen \gls{afl} dadurch, dass es speziell für die Analyse von Netzwerkprotokollen
entwickelt wurde.
Die Kernfunktionalität von \gls{afl} bleibt jedoch erhalten und wird um die Möglichkeit erweitert, Netzwerkprotokolle zu
analysieren.\newline
Hierzu müssen die Netzwerkpakete, die an das \gls{zup} gesendet werden, in einem speziellen Format vorliegen.
Die Extraktion der Netzwerkpakete erfolgt mithilfe eines \gls{pcap}-Dump-Files.
Diese Dump-Files enthalten den Netzwerkverkehr, der zwischen zwei Kommunikationspartnern stattgefunden hat.
Um diese Dump-Files zu generieren und somit den Netzwerkverkehr aufzunehmen wird das Tool \texttt{tcpdump} verwendet.
Zur Generierung der Netzwerkpakete wird das \gls{pcap}-Dump-File in ein \gls{afl}-kompatibles Format umgewandelt, indem
die rohen Datenflüsse extrahiert und in eine Datei geschrieben werden.
Diese Datenflüsse werden dann von \gls{afl} als Eingabe verwendet, um das \gls{zup} zu fuzzen.
\gls{afl}Net verfügt über eine von \gls{afl} abweichende Architektur. \textit{hier zitieren}

\subsubsection{Setup}
\begin{itemize}
    \item Hier beschreiben wie \gls{afl} konfiguriert wurde, sodass man das \gls{zup} fuzzen kann.
    \item beschreiben wie die Eingaben generiert wurden.
    \item Testcases beschreiben.
    \item Auf PA eingehen
\end{itemize}
Da \gls{afl}Net legacy Software ist, wird es nicht mehr aktiv weiterentwickelt.
Zum Kompilieren von \gls{afl}Net wird hierzu ein angepasstes Dockerfile verwendet. \textit{Zitieren von Repo}
Dieses angepasste Dockerfile enthält alle notwendigen Abhängigkeiten, um \gls{afl}Net zu kompilieren.
Hierzu zählen die Abhängigkeiten \texttt{llvm} und \texttt{clang} in der Version 11.
\texttt{llvm} wird benötigt, um den \gls{afl} Compiler \gls{afl-clang-fast} zu kompilieren.
Dieser wird für optimierte Instrumentierung des Codes verwendet und bezweckt somit schnellere Ausführgeschwindigkeiten
während der Fuzzing Kampagne.
\subsubsection{Analyse der Effektivität gesendeter Pakete}
- Hier eine kurze Begründung, dass AFLNet nicht MQTT nur mit einer bestehenden Connection Fuzzen kann. \newline
- Persistent mode vielleicht einbauen, sodass MQTT nicht immer wieder erneut gestartet wird und den erlangten State resettet.
- Schlussfolgern, dass AFLNet für MQTT besser angepasste werden muss.

%\subsubsection{Analyse der Geschwindigkeit}
%\begin{itemize}
%    \item Hier wird die Geschwindigkeit von \gls{afl} analysiert.
%    \item Wie viele Testcases wurden generiert.
%    \item Wie viele Testcases wurden pro Sekunde generiert.
%    \item Wie viele Testcases wurden pro Sekunde an das \gls{zup} gesendet.
%\end{itemize}
%\subsubsection{Analyse der gefundenen Bugs}
%\begin{itemize}
%    \item Eingehen wieviele Bugs gefunden wurden
%    \item Verifizieren
%    \item Auf reproduzierbarkeit eingehen
%\end{itemize}


