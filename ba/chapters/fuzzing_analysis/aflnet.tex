...
\gls{afl}Net verfügt über eine von \gls{afl} abweichende Architektur. \textcolor{red}{hier zitieren und bild einfügen}
\begin{figure}[H]
    \centering
...
\begin{itemize}
    ...
    \item V.a. auf die Extraktion der Nachrichten eingehen mit Wireshark UND danach das auf das Skript schlussfolgern
\end{itemize}
}
...
\end{figure}
\noindent Bei der Analyse der Fehleranfälligkeit von nicht validen Nachrichten wurde festgestellt, dass am Anfang der Fuzzing-Kampagne
...
\end{figure}
\noindent Daraus ist schlusszufolgern, dass der Fuzzer \gls{afl}Net in der Lage ist, valide Nachrichten un deren Struktur anhand
...
\end{figure}
Die Ausführungsrate ist eine zentrale Metrik, um den Fortschritt und die Effektivität einer Fuzzing-Kampagne zu bewerten.
...
\end{itemize}
Ein leichtes Absinken der Ausführungsrate zwischen dem 24. August, 12:00 Uhr, und dem 25. August, 00:00 Uhr, ist bemerkenswert.
...
\end{figure}
Das gezeigte Diagramm veranschaulicht den Verlauf einer 48-stündigen Fuzzing-Kampagne mit dem \gls{afl}Net-Tool, wobei
...
Die graue Fläche~\ref{fig:aflnet_speed} zeigt die Gesamtanzahl der explorierten Pfade während der Kampagne.
...
Die hellblaue Linie~\ref{fig:aflnet_speed} repräsentiert den aktuell im Test befindlichen Pfad.
...
Die Anzahl der noch zu testenden Pfade (blau~\ref{fig:aflnet_speed}) zeigt einen Anstieg, der während der gesamten Kampagne
...
Die lila Kurve~\ref{fig:aflnet_speed} stellt die favorisierten Pfade dar, die noch nicht getestet wurden.
Diese Zahl steigt ebenfalls stetig an, jedoch langsamer als die \enquote{pending paths}~\ref{fig:aflnet_speed}.
...
Die magentafarbene Kurve~\ref{fig:aflnet_speed} zeigt die Anzahl der abgeschlossenen Testzyklen.
...
\end{figure}
Die Anzahl der einzigartigen Abstürze (pink~\ref{fig:aflnet_bugs}) zeigt einen kontinuierlichen Anstieg über den gesamten Kampagnenzeitraum.
...
Die Anzahl der einzigartigen Hänger (magenta~\ref{fig:aflnet_bugs}) bleibt über den gesamten Kampagnenzeitraum stabil bei null.
...
Die Levels-Kurve (blau~\ref{fig:aflnet_bugs}) zeigt nur einen leichten Anstieg in den ersten Stunden der Kampagne und bleibt danach weitgehend konstant.
...
Die Fuzzing-Kampagne mit AFLNet war erfolgreich darin, eine beträchtliche Anzahl einzigartiger Abstürze zu entdecken,
