...
Die einfachste Schwachstelle ist ein Buffer Overflow in der Funktion \texttt{mqtt\_handle\_connect}~\ref{lst:handle-connect} und wird genau dann
getriggert, wenn ein Client eine Verbindung zum Broker aufbaut und der Client im Payload über eine \texttt{will}-Nachricht
verfügt.
Die mittlere Schwachstelle ist ein Integer Overflow in der Funktion \texttt{mqtt\_handle\_publish}~\ref{lst:handle-publish} und wird getriggert,
...
Die komplexeste Schwachstelle ist ein Memory Leak in der Funktion \texttt{mqtt\_handle\_subscribe}~\ref{lst:handle-subscribe} und wird getriggert,
